...
if mode == "Stable Diffusion":
    ...
    if send:
        ...
        if not prompt == "":
            ...
            request = chatglm_json(str(f"不需要你作画，不需要给我参考，不需要你给我形容你的作画内容，请给出“{prompt_text}”中的作画内容，请直接给出作画内容和对象"),
            ...
            request = chatglm_json(f"请介绍一下你画的关于{draw_object}", prompt_history, int(max_length), float(top_p),
                                   float(temperature))
...
if mode == "CLIP":
    ...
    uploaded_file = st.sidebar.file_uploader("Image",
